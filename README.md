...
Codex 推荐用官方 hooks 接入：把 `hooks/codex-hooks.example.json` 合并到
`~/.codex/hooks.json` 或项目 `.codex/hooks.json`，并把 `<LIGHT_DIR>` 换成项目绝对路径。
然后在 Codex CLI 里用 `/hooks` 审核并信任 hook。
Codex 官方 hooks 不是进程级事件；Light 从 `UserPromptSubmit` 开始创建会话、
切到 working 并计时。
Codex 当前没有退出/窗口关闭 hook，所以 `/hooks` 里不会出现 `SessionEnd`；本轮完成后
Light 会短暂展示完成态，然后回到“空闲”。

如果要精确检测“打开 Codex 交互式壳子”和“退出 Codex 进程”，用
`hooks/codex-wrap.cmd` 包装调用（详见 hooks 文档）。
...
└── hooks/                 # 注入 Claude Code / Codex 的脚本
    ...
    ├── codex-hook.mjs     # Codex 官方 hooks 适配器
    ├── codex-hook.cmd     # Windows 入口
    ├── codex-hook.sh      # Unix 入口
...
- Codex 官方 hooks 可拿到 turn 级 start / tool / stop；包装脚本仍只能感知 CLI 进程生命周期。
